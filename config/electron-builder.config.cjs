...
const devChannelRepo = isHourlyChannel
  ...
  : isDailyChannel
    ...
    : isAdhocChannel
      ? 'orca-adhoc'
      : null
const updateFeedOwner = process.env.ORCA_UPDATE_OWNER || 'stablyai'
const updateFeedRepo = process.env.ORCA_UPDATE_REPO || devChannelRepo || 'orca'
...
module.exports = {
  ...
  win: {
    ...
    signtoolOptions: {
      ...
      ...(isUnsignedWinUpdateBuild ? {} : { publisherName: 'SignPath Foundation' })
    },
    ...(isUnsignedWinUpdateBuild ? { verifyUpdateCodeSignature: false } : {}),
    extraResources: [
